                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-lg font-semibold text-gray-900">
                        {room.requestId.requirementDescription.length > 60
                          ? room.requestId.requirementDescription.substring(0, 60) + '...'
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-600 mb-2">
                      <span>
                        📞 With: <strong>{otherParticipant?.name}</strong> ({otherParticipant?.role})
                      </span>
                      <span>
                    ...
                    {lastMessage && (
                      <div className="text-sm text-gray-500">
                        💬 Last message: <em>"{lastMessage.message.length > 50 
                          ? lastMessage.message.substring(0, 50) + '...'
                          : lastMessage.message
                        }"</em>
                        <span className="ml-2">- {formatDate(lastMessage.timestamp)}</span>
                      </div>
                    )}
                  </div>
